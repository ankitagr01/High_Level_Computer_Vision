...
print('Number of iterators: ', num_iters)
...
# Below is the grid search code for the selection of best hyperparameters.
"""
hidden_sizes = [50, 60, 70]
num_iters = [1000, 2000, 3000]
batch_sizes = [400, 500, 600]
regs = [0.25, 0.30, 0.35]
c = 1
max = 0


def param_combinations(X_train, y_train, X_val, y_val,
                       num_iters, batch_size, learning_rate, learning_rate_decay, reg, c):
    net = TwoLayerNet(input_size, hidden_size, num_classes)
    # Train the network
    stats = net.train(X_train, y_train, X_val, y_val,
                      num_iters=num_iters, batch_size=batch_size,
...
for hidden_size in hidden_sizes:
    for num_iter in num_iters:
        for batch_size in batch_sizes:
            for reg in regs:
                val_accuracy = param_combinations(X_train, y_train, X_val, y_val,
...
print("Max accuracy for model no  : ", max_for_model)
